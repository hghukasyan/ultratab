...
/**
 * Run a single async pass and collect:
 * - Wall time (ns)
 * - Peak RSS (bytes) during run
 * - CPU user/system (microseconds) delta
 * - Event loop delay histogram (min, max, mean, p50, p95)
 * - Row count (for throughput)
 */
async function measureRun(fn, { sampleRssIntervalMs = 50 } = {}) {
  const rssSamples = [];
  let rssInterval;
