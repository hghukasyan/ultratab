...
async function measureCsv(filePath: string, batchSize: number): Promise<{ rows: number; ms: number }> {
  let totalRows = 0;
...
async function measureCsvColumns(filePath: string, batchSize: number): Promise<{ rows: number; ms: number }> {
  let totalRows = 0;
  const start = process.hrtime.bigint();
