...
const {
  ...
  destroyParser,
  getParserMetrics,
  createColumnarParser,
  getNextColumnarBatch,
  destroyColumnarParser,
...
function createTempCsv(rows: number, cols = 2): string {
  const lines: string[] = [];
...
describe("arena memory", () => {
  it("row parser: arena metrics present and arena_resets increases with batches", async () => {
    ...
    try {
      ...
      while ((await getNextBatch(parser)) !== undefined) {
        const m = getParserMetrics(parser) as Record<string, number>;
  ...
  it("row parser: long streaming parse keeps arena memory bounded", async () => {
    ...
    try {
      ...
      while ((await getNextBatch(parser)) !== undefined) {
        ...
        if (bytes > maxBytesSeen) maxBytesSeen = bytes;
  ...
  it("columnar parser: arena metrics and bounded memory over many batches", async () => {
    ...
    try {
      ...
      while ((await getNextColumnarBatch(parser)) !== undefined) {
        const m = getColumnarParserMetrics(parser) as Record<string, number>;
  ...
  it("stress: columnar many rows", async () => {
    ...
    const header = "a,b,c";
    const lines = [header];
    for (let i = 0; i < dataRows; i++) lines.push(`${i},${i * 2},${i % 10}`);
