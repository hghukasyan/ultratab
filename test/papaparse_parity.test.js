...
function normalizeRows(rows) {
    return rows.map((r) => (Array.isArray(r) ? r : Object.keys(r).map((k) => r[k] ?? "")));
}
function dropTrailingEmptyRow(rows) {
    if (!rows.length)
        return rows;
...
function rowsEqual(papaRows, ultraRows, msg) {
...
describe("PapaParse parity", () => {
    it("simple unquoted CSV matches PapaParse", async () => {
        ...
        await withTempCsv(content, async (tmp) => {
            const papa = papaparseParse(content, { header: false });
    ...
    it("header: true skips first row and yields data rows only", async () => {
        const content = "x,y,z\n10,20,30\n40,50,60\n";
        await withTempCsv(content, async (tmp) => {
            const papaRaw = papaparseParse(content, { header: true, skipEmptyLines: true });
    ...
    it("tab delimiter matches", async () => {
        const content = "a\tb\tc\n1\t2\t3\n";
        await withTempCsv(content, async (tmp) => {
            const papa = papaparseParse(content, { delimiter: "\t", header: false });
