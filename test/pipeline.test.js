...
async function collectBatches(iterable, maxBatches = Infinity) {
    const batches = [];
...
describe("pipeline", () => {
    it("backpressure: queue bounded with maxQueueBatches=1", async () => {
        ...
        const batches = await collectBatches(csv(p, { batchSize: 10000, maxQueueBatches: 1 }));
        assert.ok(batches.length >= 1);
        assert.ok(batches.length <= 6);
        const totalRows = batches.reduce((s, b) => s + b.length, 0);
    ...
    it("early cancel columnar: return() stops", async () => {
        const p = ensureFixture();
        const it = csvColumns(p, { batchSize: 5000, schema: { a: "int32", b: "int32" } })[Symbol.asyncIterator]();
        const { value: first } = await it.next();
        assert.ok(first && typeof first.rows === "number");
        const result = await it.return?.();
        assert.strictEqual(result?.done, true);
    });
    it("metrics exposed when getParserMetrics called", async () => {
        const { createParser, getNextBatch, destroyParser, getParserMetrics: getMetrics, } = require("../index.js");
        const p = ensureFixture();
