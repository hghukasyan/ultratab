...
describe("xlsx streaming", () => {
    ...
    it("schema produces columnar rows", async () => {
        const batches = await collectBatches(xlsx(fixturePath, { schema: { Name: "string", Value: "int32" } }));
        assert.strictEqual(batches.length, 1);
        const b = batches[0];
        assert.deepStrictEqual(b.headers, ["Name", "Value"]);
        assert.strictEqual(b.rowsCount, 2);
    ...
    it("select filters columns", async () => {
        const batches = await collectBatches(xlsx(fixturePath, { select: ["Value"] }));
        assert.strictEqual(batches.length, 1);
        assert.deepStrictEqual(batches[0].headers, ["Value"]);
        assert.strictEqual(batches[0].rows.length, 2);
        assert.deepStrictEqual(batches[0].rows[0], ["1"]);
        assert.deepStrictEqual(batches[0].rows[1], ["2"]);
    }, { timeout: 5000 });
