...
async function main() {
    ...
    for await (const batch of csvColumns(testCsvPath, {
        ...
        console.log(`  Batch ${batchCount}: ${batch.rows} rows (total: ${totalRows})`);
        if (batch.rows > 0) {
            const idx = 0;
            const isNull = nullMaskScore?.[idx];
            console.log(`    First row: id=${ids[idx]} (Int32Array), name="${names[idx]}", score=${isNull ? "null" : scores[idx]}, active=${active[idx]}`);
        }
    }
